...
int main() {
  ...
  cudaMemcpy(reinterpret_cast<void *>(array_h),
             reinterpret_cast<void *>(array_d), 32 * sizeof(int), cudaMemcpyDeviceToHost);
  ...
  checkCudaErrors(
      cudaMalloc(reinterpret_cast<void **>(&a_d), sizeof(int32_t) * N));
  ...
  cudaMemcpy(reinterpret_cast<void *>(a_h), reinterpret_cast<void *>(a_d),
             sizeof(int32_t) * N, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
