...
void init(half *A, half *B, float *C, size_t m, size_t n, size_t k) {
  ...
  for (size_t i = 0; i < m; i++)
    for (size_t j = 0; j < n; j++)
      A[i*n + j] = static_cast<float>(rand() % 100);
}
...
int main(int argc, char *argv[]) {
  ...
  half  *B_d;
  float *C_d;
  cudaMalloc(reinterpret_cast<void **>(&A_d), m_global * k_global * sizeof(half));
  cudaMalloc(reinterpret_cast<void **>(&B_d), k_global * n_global * sizeof(half));
  cudaMalloc(reinterpret_cast<void **>(&C_d), m_global * n_global * sizeof(float));
  
  // initialize host data
  init(A_h, B_h, C_h, m_global, n_global, k_global);

  // copy host data to device
  cudaMemcpy (reinterpret_cast<void *>(A_d), reinterpret_cast<void *>(A_h), m_global *
      k_global, cudaMemcpyHostToDevice);
  cudaMemcpy (reinterpret_cast<void *>(B_d), reinterpret_cast<void *>(B_h), k_global *
      n_global, cudaMemcpyHostToDevice);
  cudaMemcpy (reinterpret_cast<void *>(C_d), reinterpret_cast<void *>(C_h), m_global *
      n_global, cudaMemcpyHostToDevice);
  ...
  cublasGemmEx(
      ...
      &alpha,
      A_d, CUDA_R_16F, m_global,
      B_d, CUDA_R_16F, k_global,
      &beta, 
      C_d, CUDA_R_32F, m_global,
      CUDA_R_32F, CUBLAS_GEMM_DFALT_TENSOR_OP);
  cudaEventRecord(stop);

  cudaMemcpy (reinterpret_cast<void *>(C_h), reinterpret_cast<void *>(C_d), m_global *
      n_global, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
