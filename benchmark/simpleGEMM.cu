...
int main(int argc, char* argv[])
{
  ...
  half* B_d;
  float* C_d;
  checkCudaErrors(cudaMalloc(reinterpret_cast<void**>(&A_d), m_global * k_global * sizeof(half)));
  checkCudaErrors(cudaMalloc(reinterpret_cast<void**>(&B_d), k_global * n_global * sizeof(half)));
  checkCudaErrors(cudaMalloc(reinterpret_cast<void**>(&C_d), m_global * n_global * sizeof(float)));
  ...
  cudaMemcpy(reinterpret_cast<void*>(B_d),
             reinterpret_cast<void*>(B_h),
             k_global * n_global,
             cudaMemcpyHostToDevice);
  cudaMemcpy(reinterpret_cast<void*>(C_d),
             reinterpret_cast<void*>(C_h),
             m_global * n_global,
             cudaMemcpyHostToDevice);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    checkCudaErrors(cublasGemmEx(cublasHandle,
                                 CUBLAS_OP_N,
                                 CUBLAS_OP_N,
                                 m_global,
                                 n_global,
  ...
  cudaMemcpy(reinterpret_cast<void*>(C_h),
